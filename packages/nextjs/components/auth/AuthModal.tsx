...
const AuthModal: React.FC<AuthModalProps> = ({ isOpen, onClose, mode }) => {
  ...
  const { signUp, signIn, signInWithGoogle, signInWithWallet } = useAuth();
  ...
  const handleWalletSignIn = async () => {
    ...
    try {
      ...
      const signature = "mock_signature"; // This should be a real signature
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-gray-50 rounded-lg p-8 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto shadow-2xl">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-900">{authMode === "signin" ? "Sign In" : "Sign Up"}</h2>
        ...
        <div className="mt-4">
          ...
          <div className="mt-4 space-y-3">
            ...
            >
              ...
              {isConnected ? `Wallet (${address?.slice(0, 6)}...${address?.slice(-4)})` : "Connect Wallet First"}
            </button>
          </div>
        </div>
