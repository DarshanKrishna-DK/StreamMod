...
export const fetchPriceFromUniswap = async (targetNetwork: ChainWithAttributes): Promise<number> => {
  // For Avalanche networks, return a mock price or fetch from a different source
  if (targetNetwork.nativeCurrency.symbol === "AVAX") {
    // Return a mock AVAX price in USD
    return 35.50; // Mock AVAX price - in production, fetch from CoinGecko or similar
  }
